...
* `:extra` - Array of subdomain frontends
    * `"subdomain"` - The subdomain to serve this frontend on - in this example, we'd be serving `diggydiggy/hole` on `dwarves.myinstance.net`
    * `"key"` - The value under `:pleroma, :frontends` to direct to, which in turn points to the directory to serve from
